...
import fr.eni.cave.dal.vin.CouleurRepository;
import fr.eni.cave.dal.vin.RegionRepository;
...
public class BouteilleServiceImpl implements BouteilleService {
	...
	private void validerBouteille(Bouteille bouteille) {
		...
		final Integer idC = bouteille.getCouleur().getId();
		if (idC == null) {
			throw new RuntimeException("Identifiant de la couleur est obligatoire");
		}

		final Integer idR = bouteille.getRegion().getId();
		if (idR == null) {
			throw new RuntimeException("Identifiant de la région est obligatoire");
		}

		final Couleur cDB = validerCouleur(idC);
		final Region rDB = validerRegion(idR);
